...
#Models 1. Consecutive. Divided into two sections, Run Models and Summarise. In 'Run Models' GLMs are run on all iterations of subsets of the data (i.e. all possible numbers of years, and all possible start years)
  ...
  #Next, there is the summarise section, which takes all the results from the model runs and compares samples to complete trends according to both sign and magnitude methods, as well as comparing different generation lengths, trend shapes etc. 
...
#If you like, you can also include data on the generation length of each species. You require a dataframe, saved as "GenLengthSpec.csv" within ResultsFP, with two columns:
  ...
  ##GenLengthMax - the 'bin' which each generation length falls into. In my case I specified 3 bins, 5 = Gen Lengths of 1-5, 10 = 6-10 and 15 = 11-15. 
    ## You can set this to whatever you like, but make sure that, for whatever bins you decide on, you adjust the lapply for Sign_All, Sign_St, Magnitude_All and Magnitude_St at the end of each Summarise section to be: 0 (which indicates include data of all genlengths) and the bin values you have chosen (e.g. 0,5,10,15)
  #If you do not want to consider generation length, in the lapply for Sign_All, Sign_St, Magnitude_All and Magnitude_St at the end of each Summarise section, remove the 5,10 and 15 and just run on 0.
#If you do not want to consider curve shape, simply do not run the Sign_EDF or Magnitude_EDF
...
#Sign function returns the number of samples that are matching, an erroneous positive/negative, missed positive/negative or opposing when compared to the complete trend
...
Sign <- function(DATA, Standardise, GenLength, EDFNum, Significance){
  Input <- DATA
...
#It then compares sample trend to complete trend according to the two comparison techniques (Sign and Magnitude)
CastConsecutive <- pblapply(c(4:MCL), function(CompleteLength){
  if((file.exists(paste0(ResultsFP, "GLM/Summaries/Sign_Consecutive_", modelfam, "_", CompleteLength,".csv"))&
      file.exists(paste0(ResultsFP, "GLM/Summaries/Magnitude_Consecutive_", modelfam, "_", CompleteLength,".csv")))==TRUE){
    return(NULL)
  } else {
    write.csv(NULL, paste0(ResultsFP, "GLM/Summaries/Sign_Consecutive_", modelfam, "_", CompleteLength,".csv"))
    ...
    Sign_All <- rbindlist(pblapply(c(0,5,10,15), function(x) Sign(AllTheStartYears, Standardise=FALSE, GenLength=x, EDFNum=0, Significance = 0.05)))
    Sign_St <- rbindlist(lapply(c(0,5,10,15), function(x) Sign(AllTheStartYears, Standardise=TRUE, GenLength=x, EDFNum=0, Significance = 0.05)))
    Sign_EDF <- rbindlist(pblapply(c(3,5,7,9), function(x) Sign(AllTheStartYears, Standardise=FALSE, GenLength=0, EDFNum=x, Significance = 0.05)))
    write.csv(rbind(Sign_All, Sign_St, Sign_EDF), file=paste0(ResultsFP, "GLM/Summaries/Sign_Consecutive_", modelfam, "_", CompleteLength,".csv"), row.names=FALSE)
...
#Summarise
if((file.exists(paste0(ResultsFP, "GLM/Summaries/Sign_Interval_", modelfam, ".csv")) &
    file.exists(paste0(ResultsFP, "GLM/Summaries/Magnitude_Interval_", modelfam, ".csv")))==FALSE){
  write.csv(NULL, paste0(ResultsFP, "GLM/Summaries/Sign_Interval_", modelfam, ".csv"))
  ...
  Sign_All <- rbindlist(pblapply(c(0,5,10,15), function(x) Sign(Interval, Standardise=FALSE, GenLength=x, EDFNum=0, Significance = 0.05)))
  Sign_St <- rbindlist(pblapply(c(0,5,10,15), function(x) Sign(Interval, Standardise=TRUE, GenLength=x, EDFNum=0, Significance = 0.05)))
  Sign_EDF <- rbindlist(pblapply(c(3,5,7,9), function(x) Sign(Interval, Standardise=FALSE, GenLength=0, EDFNum=x, Significance = 0.05)))

  write.csv(rbind(Sign_All, Sign_St, Sign_EDF), file=paste0(ResultsFP, "GLM/Summaries/Sign_Interval_", modelfam, ".csv"), row.names=FALSE)
...
#This function gives us the proportion of samples that fell into each of the Sign categories (for each complete length, subset length and interval period)
#The models sections return the raw number of samples in each category, but not the proportion, so we need to calculate
PropOverall <- function(Sign, Category){
  Sign2 <- if(SamplingType=="Consecutive"){split(Sign, Sign$CompleteLength)}else{split(Sign, Sign$NumYears)} #Split into a list of dataframes, one for each Complete length (consecutive sampling) or number of years sampled (interval sampling)
  DataFrame <- rbindlist(lapply(Sign2, function(y){
    okcast <- if(SamplingType=="Consecutive"){
      dcast(y, NumYears~variable, value.var="value") #Cast so that the Sign categories are put as columns, with the length of samples (Numyears) as the rows, and cell values the number of populations
    } else {
...
#This function gives us the proportion of samples that fell into each of the Sign categories (for each complete length, subset length and interval period), combining Missed positive and negative into one term, ditto for Erroneous positive and negative
PropOverallCombine <- function(Sign, Category){
  Sign2 <- if(SamplingType=="Consecutive"){split(Sign, Sign$CompleteLength)}else{split(Sign, Sign$NumYears)} #Split into a list of dataframes, one for each Complete length (consecutive sampling) or number of years sampled (interval sampling)
  DataFrame <- rbindlist(lapply(Sign2, function(y){
    okcast <- if(SamplingType=="Consecutive"){
      dcast(y, NumYears~variable, value.var="value") #Cast so that the Sign categories are put as columns, with the length of samples (Numyears) as the rows, and cell values the number of populations
...
#General plot function for sign data
SignPlot <- function(Sign){ 
  Sign$Prop <- round_any(Sign$Prop*100, 10, floor) #Convert proportions to percentages and round down to nearest 10
  Sign$Prop <- paste0(Sign$Prop, "-", Sign$Prop+10) #Create label (e.g. "0-10" rather than just "0")
  Sign$Prop <- factor(Sign$Prop, levels=sapply(seq(0,90,10), function(x) paste0(x, "-", x+10))) #Convert to factor
  ...
  if(SamplingType=="Consecutive"){ #Create labels (adjusts based on consecutive or interval sampling)
    ...
    aspectrat <- 0.52
    ex <- Sign$NumYears
    why <- Sign$IntervalLength
  }
  
  ggplot(Sign, aes(x=ex,y=why))+
    geom_tile(aes(fill=Prop), colour = "grey40",size = 0.01)+ #Tile makes the nice squares
    facet_wrap(~Category, nrow=2)+ #Add a facet for each sign category
    ylab(ylabel)+
    xlab(xlabel)+
...
#Plot function for sign data split by generation length
SignPlotGen <- function(Sign){
  Sign$Prop <- round_any(Sign$Prop*100, 10, floor) #Convert proportions to percentages and round down to nearest 10
  Sign$Prop <- paste0(Sign$Prop, "-", Sign$Prop+10) #Create label (e.g. "0-10" rather than just "0")
  Sign$Prop <- factor(Sign$Prop, levels=sapply(seq(0,90,10), function(x) paste0(x, "-", x+10))) #Convert to factor
  ...
  Sign$GenLength <- as.character(as.numeric(Sign$GenLength)) #This block adjusts all the names so the plot works
  Sign[Sign$GenLength==5,]$GenLength <- "a) Short"
  Sign[Sign$GenLength==10,]$GenLength <- "b) Medium"
  Sign[Sign$GenLength==15,]$GenLength <- "c) Long"
  Sign[Sign$Category=="a) Matching",]$Category <- "Matching"
  Sign[Sign$Category=="c) Erroneous +/-",]$Category <- "Erroneous +/-"
  Sign[Sign$Category=="d) Missed +/-",]$Category <- "Missed +/-"
  Sign[Sign$Category=="b) Opposing",]$Category <- "Opposing"
  Sign$GenLength <- factor(Sign$GenLength)
  Sign$GenLength <- factor(Sign$GenLength,levels(Sign$GenLength)[c(1,2,3)])
  Sign$Category <- factor(Sign$Category)
  Sign$Category <- factor(Sign$Category,levels(Sign$Category)[c(2,4,1,3)])
...
#Plot function for sign data split by trend shape
SignPlotEDF <- function(Sign){
  Sign$Prop <- round_any(Sign$Prop*100, 10, floor) #Convert proportions to percentages and round down to nearest 10
  Sign$Prop <- paste0(Sign$Prop, "-", Sign$Prop+10) #Create label (e.g. "0-10" rather than just "0")
  Sign$Prop <- factor(Sign$Prop, levels=sapply(seq(0,90,10), function(x) paste0(x, "-", x+10))) #Convert to factor
  ...
  Sign[Sign$Category=="a) Matching",]$Category <- "Matching" #This block adjusts all the names so the plot works
  Sign[Sign$Category=="c) Erroneous +/-",]$Category <- "Erroneous +/-"
  Sign[Sign$Category=="d) Missed +/-",]$Category <- "Missed +/-"
  Sign[Sign$Category=="b) Opposing",]$Category <- "Opposing"
  Sign$EDF <- as.character(as.numeric(Sign$EDF))
  Sign[Sign$EDF==3,]$EDF <- "a) 1-3"
  Sign[Sign$EDF==5,]$EDF <- "b) 3-5"
  Sign[Sign$EDF==7,]$EDF <- "c) 5-7"
  Sign[Sign$EDF==9,]$EDF <- "d) 7-9"
  Sign$EDF <- factor(Sign$EDF)
  Sign$EDF <- factor(Sign$EDF,levels(Sign$EDF)[c(1,2,3,4)])
  Sign$Category <- factor(Sign$Category)
  Sign$Category <- factor(Sign$Category,levels(Sign$Category)[c(2,4,1,3)])
...
MagnitudePlot <- function(Sign){
  ...
  Sign <- Sign[Sign$Tolerance %in% MagnitudeSub,] #Subset to the tolerances we want to plot (I modelled more just out of interest)
  Sign$Tolerance <- paste0("± ", Sign$Tolerance) #Add a plusminus sign so labels are correct
  if(SamplingType=="Consecutive"){
    Sign$CompleteLength <- as.character(as.numeric(Sign$CompleteLength))
  }
  if(SamplingType=="Interval"){
    Sign$CompleteLength <- Sign$IntervalLength #Cheeky relabel of interval length to be complete length so that this function can be applied to both data (bit janky but it works, and labels ensure it's labelled as interval length in the actual plot)
  }
  Sign$Tolerance <- as.factor(Sign$Tolerance)
  if(SamplingType=="Consecutive"){ #Add labels to the years (consecutive)
    Sign$CompleteLength <- sapply(Sign$CompleteLength, function(x){
      if(x==5){
        paste0("a) ", x, " years")
      } else if(x==10){
        paste0("b) ", x, " years")
    ...
    Sign$CompleteLength <- factor(Sign$CompleteLength, levels=c("a) 5 years","b) 10 years","c) 20 years","d) 30 years"))
  } else {
    Sign$CompleteLength <- sapply(Sign$CompleteLength, function(x){ #Add labels to the years between samples (intervals)
      if(x==1){
    ...
    Sign$CompleteLength <- factor(Sign$CompleteLength, levels=c("a) Samples taken every 1 year","b) Samples taken every 3 years","c) Samples taken every 5 years","d) Samples taken every 7 years","e) Samples taken every 9 years","f) Samples taken every 11 years"))
  }
...
MagnitudePlotGen <- function(Sign){
  MagnitudeSub <- c(0.01,0.05, 0.1) #Subset to the tolerances we want to plot 
  Sign <- Sign[Sign$Tolerance %in% MagnitudeSub,] #Subset to the tolerances we want to plot
  Sign$Tolerance <- paste0("± ", Sign$Tolerance) #Add plus minus so the labels are correct
  Sign$GenLength <- as.character(as.numeric(Sign$GenLength))
  Sign[Sign$GenLength==5,]$GenLength <- "a) Short" #Get the labels correct
  Sign[Sign$GenLength==10,]$GenLength <- "b) Medium"
  Sign[Sign$GenLength==15,]$GenLength <- "c) Long"
  Sign$GenLength <- factor(Sign$GenLength)
  Sign$GenLength <- factor(Sign$GenLength,levels(Sign$GenLength)[c(1,2,3)])
  ...
  if(SamplingType=="Consecutive"){ #Add labels
    Sign$CompleteLength <- paste0(Sign$CompleteLength, " Years")
    Sign$CompleteLength <- factor(Sign$CompleteLength, levels=c("10 Years","20 Years","30 Years"))
  } else {
    Sign$CompleteLength <- sapply(Sign$CompleteLength, function(x){
      if(x==1){
  ...
  breaksx <- seq(0,MCL,5)
  Sign$PropCorrect <- Sign$PropCorrect*100 #Change proportion to percentage
  ggplot(Sign, aes(x=NumYears,y=PropCorrect, colour=GenLength))+ #Make a line plot!
    geom_line()+
...
MagnitudePlotEDF <- function(Sign){
  MagnitudeSub <- c(0.01,0.05, 0.1) #Subset to the tolerances we want to plot (I modelled more just out of interest)
  Sign <- Sign[Sign$Tolerance %in% MagnitudeSub,] #Subset to the tolerances we want to plot (I modelled more just out of interest)
  Sign$Tolerance <- paste0("± ", Sign$Tolerance) #Add a plusminus sign so labels are correct
  Sign$EDF <- as.character(as.numeric(Sign$EDF))
  Sign[Sign$EDF==3,]$EDF <- "1-3"
  Sign[Sign$EDF==5,]$EDF <- "3-5"
  Sign[Sign$EDF==7,]$EDF <- "5-7"
  Sign[Sign$EDF==9,]$EDF <- "7-9"
  Sign$EDF <- factor(Sign$EDF)
  Sign$EDF <- factor(Sign$EDF,levels(Sign$EDF)[c(1,2,3,4)])
  if(SamplingType=="Consecutive"){
    Sign$CompleteLength <- as.character(as.numeric(Sign$CompleteLength))
  ...
  breaksx <- seq(0,MCL,5)
  Sign$PropCorrect <- Sign$PropCorrect*100 #Change proportion to percentage
  ggplot(Sign, aes(x=NumYears,y=PropCorrect, colour=EDF))+ #Make a line plot!
    geom_line()+
    geom_point(shape=18, size=1.5)+
    facet_grid(Tolerance~CompleteLength)+
    ylab("Percentage within tolerance")+
...
SignSummary <- rbindlist(lapply(list.files(path=paste0(ResultsFP, "GLM/Summaries/"), pattern=paste0("^Sign_Consecutive_",modelfam, "..*.csv$"), full.names=TRUE), fread), fill=TRUE) #Read in summarised CONSECUTIVE data from model runs
...
SignSummaryInterval <- read.csv(file=paste0(ResultsFP, "GLM/Summaries/Sign_Interval_", modelfam, ".csv")) #Read in summarised data from model runs
SignSummaryComplete <- subset(SignSummary, CompleteLength==MCL) #Extract MCL data from consecutive data (to add to intervals)
SignSummaryComplete$IntervalLength <- 1 #Set the interval length at one
SignSummaryComplete$CompleteLength <- NULL #Remove complete length (they're all MCL)
SignSummaryInterval <- rbind(SignSummaryComplete, SignSummaryInterval) #Add MCL data to interval data
...
for(SamplingType in c("Consecutive", "Interval")){ #Loop through consecutive and interval
  for(Casttype in c("All", "St")){ #Loop through all data ("All") and standardised data ("St", which checks for biases)
    if(SamplingType=="Consecutive"){ #Subset to relevant category and type
      Sign <- subset(SignSummary, CastType==Casttype)
    } else {
      Sign <- subset(SignSummaryInterval, CastType==Casttype)
    }
    
    #All
    SignAll <- subset(Sign, GenLength==0 & EDF==0) #Remove data subsetted by Generation Length or Trend Shape
  ...
  if(SamplingType=="Consecutive"){
    Sign <- subset(SignSummary, CastType=="All")
  } else {
    Sign <- subset(SignSummaryInterval, CastType=="All")
  }
  
  #Generation length
  SignGen <- subset(Sign, GenLength!=0 & EDF==0) #Subset to Generation Length categories (but not EDF)
  SignPlotsGen <- rbindlist(lapply(c(5,10,15), function(Gen){ #Apply PropOverallCombine within each generation length
    gendata <- subset(SignGen, GenLength==Gen)
  ...
  #EDF
  SignEDF <- subset(Sign, GenLength==0 & EDF!=0) #Subset to EDF categories (but not generation length)
  SignPlotsEDF <- rbindlist(lapply(unique(SignEDF$EDF), function(edf){ #Apply PropOverallCombine within each EDF category
    edfdata <- subset(SignEDF, EDF==edf)
...
PropOverallSuppData <- function(Sign, Category){
  Sign2 <- if(SamplingType=="Consecutive"){split(Sign, Sign$CompleteLength)}else{split(Sign, Sign$NumYears)}
  DataFrame <- rbindlist(lapply(Sign2, function(y){
    okcast <- if(SamplingType=="Consecutive"){
...
SignSummary <- rbindlist(lapply(list.files(path=paste0(ResultsFP, "GLM/Summaries/"), pattern=paste0("^Sign_Consecutive_",modelfam, "..*.csv$"), full.names=TRUE), fread), fill=TRUE) #Read in data
...
SignSummaryInterval <- read.csv(file=paste0(ResultsFP, "GLM/Summaries/Sign_Interval_", modelfam, ".csv")) #As in plots section, add MCL data to intervals
SignSummaryComplete <- subset(SignSummary, CompleteLength==MCL)
SignSummaryComplete$IntervalLength <- 1
SignSummaryComplete$CompleteLength <- NULL
SignSummaryInterval <- rbind(SignSummaryComplete, SignSummaryInterval)
...
for (SamplingType in c("Consecutive", "Interval")){ # Loop through consecutive and interval
  ...
  SignAll <- subset(Sign, GenLength==0 & EDF==0) #First get proportions from all data
  SignPlots <- rbindlist(pblapply(c("Matching","Opposing", "MPos","MNeg", "EPos", "ENeg"), function(Category) PropOverallSuppData(SignAll, Category))) #Apply PropOverallSuppData function
  
  SignGen <- subset(Sign, GenLength!=0 & EDF==0) #Repeat for Generation length data
  SignPlotsGen <- rbindlist(lapply(c(5,10,15), function(Gen){
    gendata <- subset(SignGen, GenLength==Gen)
  ...
  SignEDF <- subset(Sign, GenLength==0 & EDF!=0) #Repeat for trend shape data
  SignPlotsEDF <- rbindlist(lapply(unique(SignEDF$EDF), function(edf){
    edfdata <- subset(SignEDF, EDF==edf)
  ...
  SignPlotsGen$EDF <- NA
  SignPlotsEDF$GenLength <- NA
  SignPlots[,c("EDF", "GenLength")] <- NA
...
SignInsig <- function(DATA, Standardise, GenLength, EDFNum, Significance){
  Input <- DATA
  SamplingType <- unique(Input$SamplingType)
  #Reduce the data according to the various subsets
  if(GenLength==0){
...
#The following function organises the data so that samples can be compared to multiple complete trend lengths. E.g. A complete trend 10 years long taken from years 10-19 could have 3 year samples taken from it starting from years 10,11,12...17 etc)
#It then compares sample trend to complete trend according to the two comparison techniques (Sign and Magnitude), but modified for this supplementary section
CastConsecutive <- pblapply(c(4:MCL), function(CompleteLength){
  if((file.exists(paste0(ResultsFP, "GLM/SummariesInsig/Sign_Consecutive_", modelfam, "_", CompleteLength,".csv"))&
      file.exists(paste0(ResultsFP, "GLM/SummariesInsig/Magnitude_Consecutive_", modelfam, "_", CompleteLength,".csv")))==TRUE){
    return(NULL)
  } else {
    write.csv(NULL, paste0(ResultsFP, "GLM/SummariesInsig/Sign_Consecutive_", modelfam, "_", CompleteLength,".csv"))
    ...
    Sign_All <- rbindlist(pblapply(c(0,5,10,15), function(x) SignInsig(AllTheStartYears, Standardise=FALSE, GenLength=x, EDFNum=0, Significance = 0.05)))
    Sign_St <- rbindlist(lapply(c(0,5,10,15), function(x) SignInsig(AllTheStartYears, Standardise=TRUE, GenLength=x, EDFNum=0, Significance = 0.05)))
    Sign_EDF <- rbindlist(pblapply(c(3,5,7,9), function(x) SignInsig(AllTheStartYears, Standardise=FALSE, GenLength=0, EDFNum=x, Significance = 0.05)))
    write.csv(rbind(Sign_All, Sign_St, Sign_EDF), file=paste0(ResultsFP, "GLM/SummariesInsig/Sign_Consecutive_", modelfam, "_", CompleteLength,".csv"), row.names=FALSE)
...
### Summarise Interval
if((file.exists(paste0(ResultsFP, "GLM/SummariesInsig/Sign_Interval_", modelfam, ".csv")) &
    file.exists(paste0(ResultsFP, "GLM/SummariesInsig/Magnitude_Interval_", modelfam, ".csv")))==FALSE){
  write.csv(NULL, paste0(ResultsFP, "GLM/SummariesInsig/Sign_Interval_", modelfam, ".csv"))
  ...
  Sign_All <- rbindlist(pblapply(c(0,5,10,15), function(x) SignInsig(Interval, Standardise=FALSE, GenLength=x, EDFNum=0, Significance = 0.05)))
  Sign_St <- rbindlist(pblapply(c(0,5,10,15), function(x) SignInsig(Interval, Standardise=TRUE, GenLength=x, EDFNum=0, Significance = 0.05)))
  Sign_EDF <- rbindlist(pblapply(c(3,5,7,9), function(x) SignInsig(Interval, Standardise=FALSE, GenLength=0, EDFNum=x, Significance = 0.05)))
  
  write.csv(rbind(Sign_All, Sign_St, Sign_EDF), file=paste0(ResultsFP, "GLM/SummariesInsig/Sign_Interval_", modelfam, ".csv"), row.names=FALSE)
...
#Read in summaries
SignSummary <- rbindlist(lapply(list.files(path=paste0(ResultsFP, "GLM/SummariesInsig/"), pattern=paste0("^Sign_Consecutive_",modelfam, "..*.csv$"), full.names=TRUE), fread), fill=TRUE) #Read in summarised CONSECUTIVE data from model runs
...
MagnitudePlot <- function(Sign){
  ...
  if(SamplingType=="Consecutive"){ #Add labels to the years (consecutive)
    ...
    Sign$CompleteLength <- factor(Sign$CompleteLength, levels=c("a) 10 years","b) 20 years","c) 30 years"))
  } else {
    Sign$CompleteLength <- sapply(Sign$CompleteLength, function(x){ #Add labels to the years between samples (intervals)
      if(x==1){
    ...
    Sign$CompleteLength <- factor(Sign$CompleteLength, levels=c("a) Samples every 1 yr","b) Samples every 3 yrs","c) Samples every 6 yrs"))
  }
  if(SamplingType=="Consecutive"){ #Set breaks for plot
    breaksx <- seq(0,MCL,5)
    breaksy <- seq(0,100,10)
...
### Build and save sign plots (supplementary insig code)
for(SamplingType in c("Consecutive", "Interval")){ #Loop through consecutive and interval
  for(Casttype in c("All", "St")){ #Loop through all data ("All") and standardised data ("St", which checks for biases)
    ...
    #Apply plot functions and save
    ggsave(paste0(FiguresFP,"Insig/Sign/Sign_", SamplingType, "_", Casttype, "_", modelfam, ".pdf"), SignPlot(SignAll), device="pdf", width = W, height = H, units = "mm") #Save as a pdf
  }
}
